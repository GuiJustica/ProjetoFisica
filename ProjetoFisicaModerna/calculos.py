...
def probabilidadeParticulaE(largura, n, X):
    x = x * largura
    probabilidadeE = (2 / largura) * pow((math.sin((n * math.pi * X) / largura)), 2)
...
def probabilidadeParticulaP(largura, n, x):
    x = x * largura
    probabilidadep = (2 / largura) * pow((math.sin((n * math.pi * x) / largura)), 2)
...
def probabilidadeIntegralNi(A, k1, integralA, integralB):
    integrali =  integrate.quad((lambda x: pow(A * math.sin(k1 * x), 2)), integralA, integralB)
    porcentagemI = integrali[0] * 100
    return porcentagemI

def probabilidadeIntegralNf(A, k2, integralA, integralB):
    integralf = integrate.quad(lambda x: pow(A * math.sin(k2 * x), 2), integralA, integralB)
